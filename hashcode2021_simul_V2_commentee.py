class Car():
    ...
    def avance(self,feux,rues,cars): # gère l'avancement de la voiture à chaque itération (temps t->t+1)
        if self.dist_feu > 0 :#si la voiture n'est pas encore à un feu, on avance
            self.dist_feu -= 1
            if self.dist_feu == 0 :#ATTENTION : si la voiture se retrouve au feu, suite à l'action précédente
                self.queue=True # alors elle entre dans la queue, et on met à jour les paramètres adéquats
...
def score(rues,voitures,carrefours,duration,bonus,juge):
    ...
    for t in range(duration) : # t est le temps
        ...
        for car in cars :
            ...
            if result == 'Arrivée' : # si une voiture est arrivée au bout de son trajets
            # mise à jour selon la règle prévue dans le document hashcode_2021_online_qualification_round.pdf
