...
class Feu():
    ...
    def __str__(self):
        texte = 'Feu '+self.id + ' Queue de '+str(self.queue_length)+' voitures : Vert = '+str(self.green)+' Durée = '+str(self.duree)+'\n'
...
def score(rues,voitures,carrefours,duration,bonus,juge):
    ...
    for id,carrefour in carrefours.items() :
        for nom in carrefour.incoming_streets :
            feux[nom]=Feu(nom,0,id,False) # on met tous les feux au rouge fixe
    ...
    for t in range(duration) : # t est le temps
        if not(juge): print ('Itération n°',t+1)
        for car in cars :
            result=car.avance(feux,rues,cars)
            if not(juge): print(car)
            if result == 'Arrivée' :
